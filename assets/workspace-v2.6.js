  ...
  function init() {
    ...
    function loadState(current) {
      ...
      if (fresh && Array.isArray(stored.drafts) && stored.drafts.length) {
        ...
        stored.executorSignerCards = Array.isArray(stored.executorSignerCards) ? stored.executorSignerCards : [];
    ...
    function renderDatalist(id, labels) {
      document.getElementById(id).replaceChildren(...labels.map(label => new Option(label)));
    }
    ...
    function renderLibraries() {
      renderDatalist('customerCardOptions', state.customerCards.map(card => card.customer));
      renderDatalist('signerCardOptions', state.signerCards.map(signerLabel));
      renderDatalist('executorSignerCardOptions', state.executorSignerCards.map(signerLabel));
      renderDatalist('serviceTemplateOptions', state.serviceTemplates);
    ...
    function addExecutorSigner(silent = false) {
      ...
      if (found) Object.assign(found, card, { updatedAt: nowIso() }); else state.executorSignerCards.unshift({ id: uid(), ...card, updatedAt: nowIso() });
    ...
    function addService(silent = false) {
      ...
      state.serviceTemplates = [service, ...state.serviceTemplates.filter(item => item !== service)].slice(0, 500);
      persistState(); renderLibraries();
    ...
    function useServiceSelection() {
      ...
      if (!value) return;
      const service = state.serviceTemplates.find(item => item === value);
      if (!service) return;
      updateField('serviceName', service);
      fields.serviceName.dispatchEvent(new Event('input', { bubbles: true }));
    }
    function useCustomerSelection() {
      const input = document.getElementById('customerCardSelect'), card = state.customerCards.find(item => item.customer === input.value);
      if (!card) return;
      updateField('customer', card.customer); fields.customer.dispatchEvent(new Event('input', { bubbles: true }));
    }
    function useSignerSelection(kind) {
      const executorSide = kind === 'executor';
      const input = document.getElementById(executorSide ? 'executorSignerCardSelect' : 'signerCardSelect');
      const source = executorSide ? state.executorSignerCards : state.signerCards;
      const card = source.find(item => signerLabel(item) === input.value);
      if (!card) return;
      if (executorSide) {
        updateField('assocPosition', card.position); updateField('assocName', card.name); updateField('assocBasis', card.basis);
        fields.assocName.dispatchEvent(new Event('input', { bubbles: true }));
      } else {
        updateField('customerPosition', card.position); updateField('customerName', card.name); updateField('customerBasis', card.basis);
    ...
    renderDrafts(); renderLibraries(); renderPrivacy(); updateContractPlaceholder();
    ...
    document.getElementById('customerCardSelect').oninput = useCustomerSelection;
    document.getElementById('signerCardSelect').oninput = () => useSignerSelection('customer');
    document.getElementById('executorSignerCardSelect').oninput = () => useSignerSelection('executor');
    document.getElementById('serviceTemplateSelect').oninput = useServiceSelection;
    ...
    window.__ACTS_WORKSPACE_TEST_API__ = Object.freeze({ semanticErrors, importFields, parseRuDate, parseBackup, backupPayload, updateContractPlaceholder, getState: () => structuredClone(state), getPreferences: () => structuredClone(prefs) });
  }
})();
